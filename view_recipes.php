    ...
    <div class="alb">
        ...
        if ($result_addresipe->num_rows > 0) {
            while ($row = $result_addresipe->fetch_assoc()) {
                echo "<h2>ADDED RECIPE</h2>";
                echo "<h2>" . $row["IngredientName"] ."</h2>";
                echo "<h3><strong>Total cost:</strong> " . $row["IngredientAmmount"] . "</h3>";
                echo "<p><strong>Number of Servings:</strong> " . $row["NumberOfServings"] . "</p>";
                echo "<p><strong>Cooking Time:</strong> " . $row["CookTime"] . "</p>";
            }
        } else {
            echo "<p>No Posted Recipes.</p>";
        }
